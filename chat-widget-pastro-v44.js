...
@media (max-width: 767px) {
    .chat-assist-widget .chat-window {
        ...
        height: 65vh !important; /* Reduzi de 80vh para 65vh */
        bottom: 0 !important;
        left: 0 !important;
        right: 0 !important;
        border-radius: 0 !important;
        /* Novas regras para landscape */
        max-height: 100vh;
    }
    ...
    @media (orientation: landscape) {
        ...
        .chat-assist-widget .chat-messages {
            height: calc(100vh - 180px) !important; /* Ajuste baseado na altura dos controles */
...
function startChat() {
    ...
    sendButton.addEventListener('click', () => {
        ...
        if (messageText && !isWaitingForResponse) {
            ...
            abrirModalCal(); // 👈 Aqui abre o Cal.com
    ...
    messageTextarea.addEventListener('keypress', (event) => {
        if (event.key === 'Enter' && !event.shiftKey) {
            ...
            if (messageText && !isWaitingForResponse) {
                submitMessage(messageText);
                abrirModalCal(); // 👈 Também aqui, para quem envia com Enter
                messageTextarea.value = '';
                messageTextarea.style.height = 'auto';
            }
        }
...
`;
  ...
  if (document.getElementById('modalCal')) return; // Evita abrir mais de uma vez
  ...
  `;
  modal.innerHTML = `
    <div style="width:90%; max-width:700px; background:white; border-radius:12px; overflow:hidden; position:relative;">
      <button onclick="document.body.removeChild(document.getElementById('modalCal'))"
              style="position:absolute; top:10px; right:10px; background:red; color:white; border:none; padding:6px 10px; cursor:pointer; border-radius:6px;">X</button>
      <iframe src="https://cal.com/thiagorspastro/30min?overlayCalendar=true" width="100%" height="600" frameborder="0"></iframe>
    </div>
  `;
