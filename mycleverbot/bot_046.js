...
(function() {
    ...
    // ========================================================================
    // == SUBSTITUA A FUNÇÃO ANTIGA 'submitPayloadToN8n' POR ESTA VERSÃO ABAIXO ==
    // ========================================================================
    const submitPayloadToN8n = async (payload) => {
        ...
        try {
            ...
            // Se a resposta for um array, pegue o primeiro elemento
            if (Array.isArray(responseData) && responseData.length > 0) {
                responseData = responseData[0];
            }

            // Agora, verifique se temos um objeto com a propriedade 'output'
            if (responseData && typeof responseData === 'object' && responseData.output && typeof responseData.output === 'string') {
                // O formato está correto, temos o texto.
                const finalOutput = responseData.output;
                addBotMessage(finalOutput);
            } else {
                // Se chegarmos aqui, o formato não é o que esperamos.
            ...
            // A função sendToN8n já exibe uma mensagem de erro em caso de problemas de rede,
            // então não precisamos adicionar outra aqui.
        } finally {
            hideThinkingIndicator();
