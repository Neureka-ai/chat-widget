...
(function() {
    ...
    widgetStyles.textContent = `
        ...
        .chat-assist-widget .chat-textarea {
            ...
            min-height: 24px;
            outline: none;
            color: var(--chat-color-text-dark); /* CORREÇÃO DA COR DA FONTE */
        }
    ...
    let currentLevel = '';
    let currentTopic = ''; // VARIÁVEL PARA GUARDAR O TEMA ATUAL
    ...
    const handleAction = (action, text) => {
        ...
        if (action === 'showLevelSelection') {
            currentTopic = ''; // Reseta o tópico ao voltar para o menu
            showLevelSelection();
            return;
        }
        ...
        if (action === 'selectTopic' || action === 'textInput') {
            if (action === 'selectTopic') {
                currentTopic = text; // Armazena o tópico selecionado
            }
            const payload = {
                message: text,
                workflow: currentLevel,
                tema: currentTopic || 'texto_livre', // Usa o tópico armazenado
    ...
    const submitPayloadToN8n = async (payload) => {
        ...
        try {
            ...
            if (responseData && responseData.output) {
                ...
                // Error already handled in sendToN8n
            } else {
                addBotMessage("Recebi sua mensagem, mas não obtive uma resposta para exibir. Tente novamente.");
            }
        } catch (error) {
