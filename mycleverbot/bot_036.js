...
(function() {
    ...
    widgetStyles.textContent = `
        ...
        @media (max-width: 1023px) {
            ...
            /* ALTERAÇÃO: Posiciona o botão no centro horizontal do ecrã, no topo */
            .chat-assist-widget .chat-launcher {
                top: 12px;
                left: 50%;
    ...
    const addBotMessage = (text, buttons = []) => {
        ...
        const isGrammarFlow = currentLevel === 'iniciante' || currentLevel === 'intermediario' || currentLevel === 'avancado';
        if (buttons.length > 0 || isGrammarFlow) {
            toggleTextInput(true, 'Por favor, selecione uma opção acima.');
        } else {
