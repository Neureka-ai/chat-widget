    ...
    const submitPayloadToN8n = async (payload) => {
        ...
        try {
            ...
            if (responseData && responseData.output) {
                // Tenta analisar a saída caso ela seja uma string JSON (problema de "JSON duplo")
                try {
                    const parsedOutput = JSON.parse(responseData.output);
                    if (parsedOutput && parsedOutput.output) {
                        finalOutput = parsedOutput.output; // Extrai o conteúdo do JSON interno
                    } else {
                        finalOutput = responseData.output;
                    }
                } catch (e) {
                    // Não é uma string JSON, então é o texto final
                    finalOutput = responseData.output;
                }
                addBotMessage(finalOutput);

            } else if (!responseData) {
                // O erro já foi tratado na função sendToN8n
            } else {
