...
(function() {
    ...
    // ALTERAÇÃO: Lógica para posicionar o botão do chatbot no mobile
    const positionMobileLauncher = () => {
        if (window.innerWidth < 1023) {
            const header = document.querySelector('.lg\\:hidden.sticky.top-0');
            if (header) {
                const headerHeight = header.offsetHeight;
                launchButton.style.top = `${headerHeight / 2}px`;
                launchButton.style.transform = 'translate(-50%, -50%)';
            }
        } else {
            // Reseta para a posição padrão em desktop
    ...
    positionMobileLauncher(); // Chama uma vez para definir a posição inicial
