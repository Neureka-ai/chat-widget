...
(function() {
    ...
    const handleAction = (action, text) => {
        ...
        if (action === 'showGrammarTopics') {
            // CORREÇÃO: Mapeia o texto do botão para o valor esperado pelo n8n
        ...
        if (action === 'selectTopic' || action === 'textInput') {
            const payload = {
                ...
                tema: (action === 'selectTopic') ? text : 'texto_livre',
    ...
    const submitPayloadToN8n = async (payload) => {
        ...
        try {
            ...
            if (responseData && responseData.output) {
                ...
                // Se sendToN8n já mostrou o erro, não fazemos nada
            } else {
    ...
    const sendToN8n = async (payload) => {
        ...
        try {
            ...
            return null; // Retorna null para indicar que o erro já foi tratado
        }
    };
