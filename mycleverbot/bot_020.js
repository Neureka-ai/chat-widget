...
(function() {
    ...
    const defaultSettings = {
        // A URL do webhook deve ser fornecida através do window.ChatWidgetConfig no HTML
        webhook: { url: '' },
        branding: {
    ...
    const handleAction = async (action, text) => {
        ...
        let workflow = 'user_interaction';
        if (action.startsWith('selectTopic')) {
            workflow = action.split('_')[1];
        }

        try {
            const responseData = await sendToN8n(text, workflow);
    ...
    const sendToN8n = async (messageText, workflowType) => {
        if (!settings.webhook.url) {
            console.warn("Webhook URL not configured. Using mock response.");
            await new Promise(resolve => setTimeout(resolve, 500));
            if (workflowType === 'gramaticaflow' || workflowType === 'treinoflow') {
                return { output: `Recebido! A processar o tópico "${messageText}".` };
            }
            return {};
        }
    ...
    const handleTextInput = () => {
        ...
        if(!text) return;
        handleAction('selectTopic_textInput', text);
