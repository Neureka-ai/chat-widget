...
(function() {
    ...
    // ========================================================================
    // == FUNÇÃO ATUALIZADA PARA DEPURAÇÃO DA RESPOSTA DO N8N ==
    // ========================================================================
    const submitPayloadToN8n = async (payload) => {
        ...
        try {
            ...
            if (responseData && typeof responseData === 'object' && responseData.hasOwnProperty('output')) {
                ...
                // SEÇÃO DE DEPURAÇÃO: Mostra a resposta bruta no chat
                console.error("Formato de resposta inesperado do n8n:", rawResponse);
                const debugMessage = `Recebi um formato inesperado. Aqui estão os dados brutos do n8n para depuração:<br><br><pre style="white-space: pre-wrap; word-wrap: break-word; background: #eee; padding: 10px; border-radius: 5px; font-size: 12px;">${JSON.stringify(rawResponse, null, 2)}</pre>`;
                addBotMessage(debugMessage);
            }
