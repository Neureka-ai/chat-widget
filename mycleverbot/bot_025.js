...
(function() {
    ...
    widgetStyles.textContent = `
        ...
        .chat-assist-widget .chat-header {
            ...
            align-items: center;
            gap: 12px;
            /* DEGRADÊ DUPLO: ROXO -> BRANCO -> AMARELO */
            background: linear-gradient(to bottom, #f5f3ff 0%, #ffffff 30%, #ffffff 70%, #fffde7 100%);
        ...
        .chat-assist-widget .chat-header-title {
            font-size: 16px; /* FONTE DO TÍTULO AJUSTADA */
            font-weight: 700;
            line-height: 1.2;
            color: var(--chat-color-text-dark);
        }
    ...
    const defaultSettings = {
        ...
        branding: {
            ...
            name: 'Do erro à excelência, palavra por palavra', // NOME ATUALIZADO
    ...
    const sendToN8n = async (message, level) => {
        if (!settings.webhook.url) {
            ...
            if (level) {
                 return { output: `### **Tópico Recebido** \n\nRecebi o seu pedido para falar sobre **${message}** no nível **${level}**. A integração com o n8n está pronta!` };
            }
            return {};
        }
