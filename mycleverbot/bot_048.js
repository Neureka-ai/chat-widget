...
(function() {
    ...
    const handleAction = (action, text) => {
        ...
        if (action === 'showGrammarTopics') {
            ...
            else if (text.includes('Intermediário')) currentLevel = 'intermediario';
            else if (text.includes('Avançado')) currentLevel = 'avancado';
    ...
    // ========================================================================
    // == FUNÇÃO ATUALIZADA PARA TRATAMENTO ROBUSTO DE RESPOSTAS (INCLUINDO JSON ANINHADO) ==
    // ========================================================================
    const submitPayloadToN8n = async (payload) => {
        ...
        try {
            const rawResponse = await sendToN8n(payload);
            
            if (rawResponse === null) {
                return; // Erro de comunicação já foi tratado pela função sendToN8n
            }
            ...
            if (Array.isArray(responseData)) {
                if (responseData.length > 0) {
                    responseData = responseData[0]; // Pega o primeiro objeto do array para processar
                } else {
                    console.log("n8n retornou um array vazio. Nenhuma mensagem para exibir.");
                    addBotMessage("Não consegui encontrar uma resposta para isso. Tente outra pergunta.");
                    return; 
                }
            }

            // A partir daqui, 'responseData' deve ser um objeto.
            // Verificamos se o objeto e a chave 'output' existem.
            if (responseData && typeof responseData === 'object' && responseData.hasOwnProperty('output')) {
                let finalOutput = '';
                const outputContent = responseData.output;

                // O conteúdo de 'output' pode ser uma string simples ou um JSON stringificado.
                // Esta lógica trata ambos os casos de forma segura.
                try {
                    // Tenta analisar o conteúdo de 'output' como se fosse JSON.
                    const parsedContent = JSON.parse(outputContent);
                    
                    // Se for um objeto e tiver uma chave 'output' dentro, usamos esse valor.
                    if (parsedContent && typeof parsedContent === 'object' && parsedContent.hasOwnProperty('output')) {
                        finalOutput = parsedContent.output;
                    } else {
                        // Se o parse funcionou mas não tem a estrutura esperada,
                        // consideramos o conteúdo original como o texto final (caso seja um JSON válido mas diferente).
                        finalOutput = outputContent;
                    }
                } catch (e) {
                    // Se o JSON.parse falhar, é porque 'outputContent' é uma string normal.
                    finalOutput = outputContent;
                }
                ...
                // Se 'responseData' não for um objeto ou não tiver a chave 'output'.
                console.error("Formato de resposta inesperado do n8n:", rawResponse);
                addBotMessage("Recebi uma resposta, mas o formato é inválido. Por favor, verifique a configuração do n8n.");
            }

        } catch (error) {
            console.error("Erro ao processar a resposta do payload:", error);
            addBotMessage("Ocorreu um erro ao processar a resposta. Tente novamente.");
        } finally {
            hideThinkingIndicator();
            isWaitingForResponse = false;
            sendButton.disabled = false;
            if (currentLevel === 'escrita') {
                toggleTextInput(false, 'Digite sua mensagem...');
            }
        }
    };
