...
(function() {
    ...
    // ========================================================================
    // == FUNÇÃO ATUALIZADA PARA MELHOR TRATAMENTO DE RESPOSTAS E ERROS ==
    // ========================================================================
    const submitPayloadToN8n = async (payload) => {
        ...
        try {
            ...
            // Verifica se a chamada ao webhook falhou (sendToN8n retorna null em caso de erro)
            if (rawResponse === null) {
                return; // A mensagem de erro já foi exibida por sendToN8n
            }

            let responseData = rawResponse;

            // Se a resposta for um array, verifica se ele tem conteúdo.
            if (Array.isArray(responseData)) {
                if (responseData.length > 0) {
                    responseData = responseData[0]; // Pega o primeiro item para processar
                } else {
            ...
            // Agora, com um objeto de dados válido, verifica se ele tem a propriedade 'output'
            if (responseData && typeof responseData === 'object' && responseData.output && typeof responseData.output === 'string') {
                const finalOutput = responseData.output;
                addBotMessage(finalOutput);
            } else {
                // Se o formato ainda assim for inesperado, exibe o erro.
            ...
            // Habilita a entrada de texto se for o fluxo de escrita
