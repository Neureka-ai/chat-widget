...
<body>
    <h1>Página de Teste do Chat Widget</h1>
    <p>O chat widget deve aparecer nesta página.</p>

    <!-- 
      CONFIGURAÇÃO DO WIDGET:
      - Use a URL do webhook.site para testar. Ela funciona sem problemas de CORS.
      - Se usar a sua própria URL e tiver erros, o problema é provavelmente a configuração CORS no seu servidor.
    -->
    <script>
        window.ChatWidgetConfig = {
            webhook: {
                url: 'https://webhook.site/a9e14c1a-7b4f-4a8f-8c3c-74a4a1b02b0f', // URL de teste que aceita CORS
                route: 'main'
            },
            branding: {
                logo: 'https://neureka-ai.com/wp-content/uploads/2024/04/logo-neureka-2-1.png',
                name: 'Assistente Neureka',
    ...
    (function() {
        ...
        const renderImages = (text) => {
            ...
            return text.replace(imgPattern, url => `<div class="image-container"><img src="${url}" class="chat-image" alt="Image from link"><span class="image-caption">Imagem</span></div>`);
        };
            ...
            iframeContainer.innerHTML = `
                ...
                <div class="message-content">
                    <div class="chat-bubble bot-bubble">
                        <p>Por favor, use o calendário abaixo para agendar uma reunião.</p>
                        <iframe 
        ...
        const startChat = () => {
            ...
            welcomeContainer.innerHTML = `
                ...
                <div class="message-content">
                    <div class="chat-bubble bot-bubble">
                        <p>Olá! 😊 Eu sou seu assistente virtual especializado em energia solar. Confira o que posso fazer por você:<br><br>
                        ...
                        4. Analisar sua conta de luz e estimar sua economia 💡💰<br><br>
            ...
            welcomeContainer.querySelectorAll('.action-button').forEach(button => {
                button.addEventListener('click', () => {
                    ...
                    if (action === 'Dúvida') {
                        submitMessage('Quero tirar uma dúvida', true); // Send user message first
                        showDoubtTopics(); // Then show options
                    } else if (action === 'Agendamento') {
                        submitMessage('Quero fazer um agendamento', true);
                        addCalendarToChat();
                    } else {
                        const messageMap = {
                            'Ticket': 'Preciso de suporte (abrir ticket)',
                            'Energia': 'Quero uma análise da minha conta de energia'
                        };
        ...
        async function submitMessage(messageText, isInternal = false) {
            ...
            try {
                ...
                if (!contentType || !contentType.includes("application/json")) {
                  throw new TypeError(`Resposta inesperada do servidor. Esperava JSON, mas recebi ${contentType}`);
                }

                const responseData = await response.json();
                
                const rawResponse = (Array.isArray(responseData) && responseData.length > 0) ? responseData[0].output : responseData.output;
                let processedResponse = renderMarkdown(rawResponse || "Sua mensagem foi recebida com sucesso!");
                ...
                let errorText = "Desculpe, ocorreu um erro inesperado. Tente novamente mais tarde.";
                if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
                    errorText = "<b>Erro de Conexão (CORS):</b> Não foi possível conectar ao servidor. Verifique se o URL do webhook está correto e se o servidor permite pedidos a partir do CodePen.";
                } else if (error instanceof TypeError) {
                    errorText = `<b>Erro de Resposta:</b> O servidor enviou uma resposta em formato inesperado. Verifique a configuração do seu webhook. (${error.message})`;
                } else if (error.message.includes('Erro de rede')) {
                     errorText = `<b>Erro de Rede (${error.message}):</b> Verifique o URL do webhook e a sua conexão à internet.`;
                }
