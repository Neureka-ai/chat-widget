...
.chat-assist-widget .chat-controls {
    ...
    // Function to detect and render images from URLs
    function renderImages(text) {
        // Image URL pattern (matches common image extensions)
        const imgPattern = /(https?:\/\/.*\.(?:png|jpg|jpeg|gif|webp|svg)(?:\?\S*)?)/gi;
        
        // Replace image URLs with img tags
        return text.replace(imgPattern, function(url) {
            return `<div class="image-container">
                <img src="${url}" class="chat-image" alt="Uploaded image">
                <span class="image-caption">Image</span>
            </div>`;
        });
    }
    ...
    async function submitMessage(messageText, isInternal = false) {
        ...
        const isImageUrl = /(https?:\/\/.*\.(?:png|jpg|jpeg|gif|webp|svg)(?:\?\S*)?)/gi.test(messageText);
        ...
        // If message contains image URL, display it
        if (isImageUrl) {
            userMessageContainer.innerHTML = `
                <div class="message-content">
                    <div class="chat-bubble user-bubble">
                        <div class="image-container">
                            <img src="${messageText}" class="chat-image" alt="Uploaded image">
                            <span class="image-caption">Imagem enviada</span>
                        </div>
                        ${fileInfo}
                    </div>
                    <span class="timestamp">${new Date().toLocaleString('pt-BR')}</span>
        ...
        try {
            if (!isInternal) { // Only send to webhook for non-internal messages
                ...
                // Process response for images and markdown
                let processedResponse = renderMarkdown(rawResponse);
                processedResponse = renderImages(processedResponse); // Process image URLs
                
                botMessageContainer.innerHTML = `
                    <img src="https://cdn-icons-png.flaticon.com/512/4712/4712035.png" class="message-icon" alt="Chatbot">
                    <div class="message-content">
                      <div class="chat-bubble bot-bubble">
                          <p>${linkifyText(processedResponse)}</p>
                      </div>
                          <span class="timestamp">${new Date().toLocaleString('pt-BR')}</span>
                    </div>
                `;
