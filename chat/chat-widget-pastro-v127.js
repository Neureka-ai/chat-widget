 ...
 //do WidgetStyles logo abaixo: Versao Neureka
    ...
    widgetStyles.textContent = `
      ...
      .chat-assist-widget .message-icon {
        ...
        /* Novo botão de reset */
        .chat-assist-widget .chat-reset-buttons-btn {
            background-color: black; /* Fundo preto */
            color: white; /* Símbolo branco */
            border: none;
            border-radius: var(--chat-radius-md);
            width: 48px;
            height: 48px;
        ...
        .chat-assist-widget .chat-reset-buttons-btn:hover {
            transform: scale(1.05);
            box-shadow: var(--chat-shadow-md);
            background-color: #333; /* Um pouco mais claro no hover */
        }

        .chat-assist-widget .chat-reset-buttons-btn svg {
            width: 22px;
            height: 22px;
        }
        ...
        .chat-assist-widget .suggested-question-btn {
            ...
            color: var(--chat-color-text);
            cursor: pointer;
            transition: all 0.2s ease; /* Adjusted for consistency */
            font-family: inherit;
            line-height: 1.4;
        }
...
.chat-assist-widget .chat-controls {
    ...
    const defaultSettings = {
        ...
        style: {
            primaryColor: '#97B4BB', // Adjusted default primary to match initial design
            secondaryColor: '#187874', // Adjusted default secondary to match initial design
            position: 'right',
    ...
    const settings = window.ChatWidgetConfig ? 
        {
            ...
            style: { 
                ...
                // Force green colors if user provided purple (keeping original logic)
    ...
    const chatInterfaceHTML = `
        <div class="chat-body">
            ...
            <div class="chat-controls">
                ...
                <button class="chat-reset-buttons-btn" title="Reativar opções iniciais">
...
function adicionarCalendarioNaConversa() {
    ...
    const resetButton = chatWindow.querySelector('.chat-reset-buttons-btn'); // Get the new reset button
    ...
    function attachActionButtonsListeners(container) {
        const buttons = container.querySelectorAll('.action-button');
        buttons.forEach(button => {
            button.addEventListener('click', () => {
                const buttonAction = button.dataset.action;
                // Disable all action buttons in ALL message containers
                document.querySelectorAll('.action-buttons-container').forEach(cont => {
                    disableActionButtons(cont);
                });
                
                // Also disable dynamic buttons if present
                const dynamicButtons = messagesContainer.querySelectorAll('.dynamic-button');
...
function startChat() {
    ...
    attachActionButtonsListeners(welcomeContainer);
   ...
   // Add styles for the action buttons
        ...
        buttonStyles.textContent = `
            ...
            .dynamic-buttons-container {
                ...
                gap: 8px;
                margin-top: 12px;
                align-self: flex-start; /* Alinha à esquerda como as mensagens do bot */
                max-width: 85%;
            }
        ...
        // Function to disable all action buttons within a given container
        function disableActionButtons(container) {
            const buttonsToDisable = container.querySelectorAll('.action-button');
        ...
        // Function to enable all initial action buttons
        function enableActionButtons() {
            // Remove any existing dynamic buttons if present
            const dynamicButtonsContainer = messagesContainer.querySelector('.dynamic-buttons-container');
            if (dynamicButtonsContainer && dynamicButtonsContainer.parentNode) {
                dynamicButtonsContainer.parentNode.removeChild(dynamicButtonsContainer);
            }

            // Create a new bot message with the initial action buttons
            const newOptionsMessage = document.createElement('div');
            newOptionsMessage.className = 'message-container bot-message';
            newOptionsMessage.innerHTML = `
                <img src="https://cdn-icons-png.flaticon.com/512/4712/4712035.png" class="message-icon" alt="Chatbot">
                <div class="message-content">
                    <div class="chat-bubble bot-bubble">
                        <p>Como posso ajudar novamente? Escolha uma opção:</p>
                        <div class="action-buttons-container">
                            <button class="action-button blue-button" data-action="Dúvida">Dúvida</button>
                            <button class="action-button green-button" data-action="Agendamento">Agendamento</button>
                            <button class="action-button orange-button" data-action="Ticket">Ticket</button>
            ...
            `;
            messagesContainer.appendChild(newOptionsMessage);
            messagesContainer.scrollTop = messagesContainer.scrollHeight;

            // Attach listeners to the newly created buttons
            attachActionButtonsListeners(newOptionsMessage);
        }
                ...
                <div class="message-content">
                    <div class="chat-bubble bot-bubble">
                        ...
                        <div class="dynamic-buttons-container">
                            ...
                            <button class="dynamic-button" data-topic="Eletromobilidade">Eletromobilidade</button>
                        </div>
                    </div>
                    <span class="timestamp">${new Date().toLocaleString('pt-BR')}</span>
                </div>
