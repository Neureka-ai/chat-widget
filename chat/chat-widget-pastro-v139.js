...
(function() {
 ...
 //do WidgetStyles logo abaixo: Versao Neureka
    ...
    widgetStyles.textContent = `
      ...
      widgetStyles.textContent += `
      /* Estilos para conteúdo de URL */
      .chat-assist-widget .chat-bubble iframe,
      .chat-assist-widget .chat-bubble video,
...
function displayURLContent(url) {
    // Cria container para o conteúdo do URL
    const urlContainer = document.createElement('div');
    urlContainer.className = 'message-container bot-message';
    
    // Verifica se é uma imagem (extensões comuns)
    const imageExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp'];
    const isImage = imageExtensions.some(ext => url.toLowerCase().includes(ext));

    // Verifica se é um vídeo (extensões comuns)
    ...
    if (isImage) {
        urlContainer.innerHTML = `
            ...
            <div class="message-content">
                <div class="chat-bubble bot-bubble">
                    <img src="${url}" style="max-width: 100%; border-radius: var(--chat-radius-sm);" alt="Imagem enviada">
                </div>
        ...
        urlContainer.innerHTML = `
            ...
            <div class="message-content">
                <div class="chat-bubble bot-bubble">
                    <video controls style="max-width: 100%; border-radius: var(--chat-radius-sm);">
        ...
        urlContainer.innerHTML = `
            ...
            <div class="message-content">
                <div class="chat-bubble bot-bubble">
                    <iframe src="${url}" width="100%" height="500" style="border-radius: var(--chat-radius-sm); border: none;">
        ...
        // Se não for um tipo reconhecido, mostra como link clicável
        urlContainer.innerHTML = `
            <img src="https://cdn-icons-png.flaticon.com/512/4712/4712035.png" class="message-icon" alt="Chatbot">
            <div class="message-content">
                <div class="chat-bubble bot-bubble">
                    <p>Conteúdo externo: <a href="${url}" target="_blank" class="chat-link">${url}</a></p>
                </div>
                <span class="timestamp">${new Date().toLocaleString('pt-BR')}</span>
            </div>
        `;
    ...
    messagesContainer.appendChild(urlContainer);
...
async function submitMessage(messageText, isInternal = false) {
    ...
    // Verifica se a mensagem é um URL
    const urlPattern = /https?:\/\/[^\s]+/gi;
    const isURL = urlPattern.test(messageText);

    // Se for um URL, exibe diretamente no chat
    if (isURL && !isInternal) {
        displayURLContent(messageText);
