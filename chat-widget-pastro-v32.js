...
// Verifica se storage está disponível
    ...
    // Fallback para quando storage não estiver disponível
    const storage = {
        set: (key, value) => {
            if (isStorageAvailable()) {
                localStorage.setItem(key, value);
            } else {
                window.tempChatStorage = window.tempChatStorage || {};
                window.tempChatStorage[key] = value;
            }
        },
        get: (key) => {
            if (isStorageAvailable()) {
                return localStorage.getItem(key);
            } else if (window.tempChatStorage) {
                return window.tempChatStorage[key];
            }
            return null;
        }
    };
