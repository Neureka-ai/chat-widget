...
widgetStyles.textContent += `
    ...
    .file-upload-label.has-file::after {
        content: '';
        position: absolute;
        top: -2px;
        right: -2px;
        ...
        border-radius: 50%;
        border: 2px solid var(--chat-color-surface);
    }
`;
...
fileInput.addEventListener('change', () => {
    ...
    if (fileInput.files.length > 0) {
        ...
        console.log('Arquivo selecionado:', fileInput.files[0].name); // Opcional para debug
    } else {
        ...
        <div class="message-content">
          ...
          Do que você precisa?</p>
                  <div class="action-buttons-container">
                    <button class="action-button blue-button">Dúvida</button>
                    <button class="action-button green-button">Agendamento</button>
                    <button class="action-button orange-button">Ticket</button>
                    <button class="action-button yellow-button">Energia</button>
                  </div>
          </div>
...
async function submitMessage(messageText) {
    ...
    if (fileInput.files.length > 0) {
        ...
        console.log('Enviando arquivo:', file.name, file.size, file.type); // Para debug
        formData.append('file', file);
        if (file.type.startsWith('image/')) {
            messageContent += `<div class="uploaded-file"><img src="${URL.createObjectURL(file)}" alt="${file.name}" style="max-width: 200px; max-height: 200px; margin-top: 10px; border-radius: 8px;"></div>`;
        } else {
            messageContent += `<div class="uploaded-file" style="margin-top: 10px; padding: 8px; background: #f0f0f0; border-radius: 8px;">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align: middle; margin-right: 8px;">
    ...
    uploadIndicator.className = 'message-container bot-message';
    uploadIndicator.innerHTML = `
        <img src="https://cdn-icons-png.flaticon.com/512/4712/4712035.png" class="message-icon" alt="Chatbot">
        <div class="message-content">
            <div class="chat-bubble bot-bubble">
                <p>Processando arquivo "${file.name}" (${(file.size / 1024).toFixed(2)} KB)...</p>
            </div>
            <span class="timestamp">${new Date().toLocaleString('pt-BR')}</span>
        </div>
    `;
    ...
    let messageContent = `<p>${messageText}</p>`;
