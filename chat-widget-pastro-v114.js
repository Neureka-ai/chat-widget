...
fileInput.addEventListener('change', () => {
  if (fileInput.files.length > 0) {
    ...
    fileButton.className = 'action-button yellow-button';
    fileButton.innerText = file.name;
    fileButton.disabled = true;

    fileNameDisplay.innerHTML = '';
    fileNameDisplay.appendChild(fileButton);

    // Feedback visual no botão de upload (opcional)
    fileInput.nextElementSibling?.style?.setProperty('background-color', 'var(--chat-color-light)');
